...
def monitor_wifi(network_name):
    try:
        ...
        result = subprocess.run(['/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport', '-I'], stdout=subprocess.PIPE, universal_newlines=True)

        # Find the line with the network name
        lines = result.stdout.split('\n')
        for line in lines:
            if ' SSID: ' + network_name in line:
                # Run the command again to get the RSSI value
                result = subprocess.run(['/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport', '-I'], stdout=subprocess.PIPE, universal_newlines=True)
                lines = result.stdout.split('\n')
                for line in lines:
                    if ' agrCtlRSSI: ' in line:
                        # Extract the signal quality
                        quality = line.split(' agrCtlRSSI: ')[1]
                        return quality
